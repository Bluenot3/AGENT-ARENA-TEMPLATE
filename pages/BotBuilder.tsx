...
import { AVAILABLE_TOOLS, IMAGE_STYLE_CHIPS, MODEL_REGISTRY, COMPATIBLE_IMAGE_MODELS, AESTHETIC_PRESETS } from '../constants';
import { 
  ...
  Database, Image as LucideImage, Layers, Globe, Eye, EyeOff, Box,
  ...
  Type as TypeIcon, Flame, RefreshCw
} from 'lucide-react';
    ...
    <div className="h-full flex flex-col animate-in fade-in duration-700">
      ...
      <header className="flex flex-col xl:flex-row xl:items-center justify-between mb-8 shrink-0 bg-slate-900/20 backdrop-blur-xl p-6 rounded-[2.5rem] border border-white/5 shadow-2xl gap-6">
        ...
        <div className="flex items-center gap-4 overflow-x-auto no-scrollbar py-1">
          <div className="flex bg-black/40 p-2 rounded-[2rem] border border-white/5">
            {STEPS.map((step, idx) => (
              <button
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-[1.5rem] transition-all whitespace-nowrap group relative ${
                  activeStep === idx 
                    ? 'bg-blue-600 text-white font-black shadow-[0_0_30px_rgba(59,130,246,0.3)] border border-blue-500/50' 
                    : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                <step.icon size={18} strokeWidth={activeStep === idx ? 3 : 2} />
                <span className="text-[11px] font-black uppercase tracking-widest hidden 2xl:block">
                  {step.label}
                </span>
                {activeStep === idx && (
          ...
          <button onClick={handleSave} className="bg-white text-black px-10 py-5 rounded-[2rem] font-black text-[13px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-2xl flex items-center gap-4 active:scale-95 border border-white/10 shrink-0">
            <Save size={20} /> Save Manifest
          </button>
        </div>
      </header>
             ...
             <div className="animate-in slide-in-from-left-4 duration-500 space-y-6">
                <div className="liquid-glass p-10 rounded-[3rem] border border-white/10 bg-gradient-to-br from-slate-900/50 to-black/50 space-y-12 shadow-2xl">
                   <div className="flex items-center gap-4 border-b border-white/5 pb-8">
                      ...
                      <h3 className="text-[14px] font-black text-white uppercase tracking-[0.3em]">Integrated Intelligence Nodes</h3>
                   </div>
                   ...
                   <div className="space-y-4">
                      {AVAILABLE_TOOLS.map((tool) => {
                         ...
                         return (
                            <div 
                              key={tool.tool_id} 
                              className={`p-6 rounded-[2rem] border transition-all duration-500 flex items-center justify-between group ${isActive ? 'bg-indigo-600/10 border-indigo-500 shadow-[0_0_40px_rgba(79,70,229,0.1)]' : 'bg-black/20 border-white/5 hover:border-white/10'}`}
                            >
                               <div className="flex items-center gap-6">
                                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-all duration-500 ${isActive ? 'bg-indigo-600 text-white border-indigo-400 shadow-xl' : 'bg-white/5 text-slate-500 border-white/10'}`}>
                                     {tool.tool_id.includes('image') ? <ImageIcon size={24} /> : tool.tool_id.includes('code') ? <Terminal size={24} /> : tool.tool_id.includes('web') ? <Globe size={24} /> : <Box size={24} />}
                                  </div>
                                  <div className="overflow-hidden">
                                     <h4 className={`text-[14px] font-black uppercase tracking-widest truncate ${isActive ? 'text-white' : 'text-slate-400'}`}>{tool.name}</h4>
                                     <p className="text-[10px] text-slate-600 font-bold uppercase mt-1 tracking-wider line-clamp-1">{tool.description}</p>
                                  </div>
                               </div>
                               <button 
                                 onClick={() => toggleTool(tool.tool_id)} 
                                 className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${isActive ? 'bg-indigo-600 text-white shadow-xl' : 'bg-white/5 text-slate-500 hover:text-white hover:bg-white/10'}`}
                               >
