...
export const ModelRouter = {
  chatStream: async (
    ...
    const systemPromptParts = [
      ...
      '\n[PROTOCOL_SUGGESTIONS]: At the very end of your response, strictly provide 2-3 short, high-value follow-up prompts that the user might want to ask next. Format them exactly as: [NEXT_IDEA: The suggestion text]. Do not number them or add preambles.',
    ...
    for await (const chunk of responseStream) {
      if (chunk.text) {
        fullText += chunk.text;
        // Strip suggestions from live stream if needed, but for simplicity we stream all, then clean up in final
    ...
    while ((match = suggestionRegex.exec(fullText)) !== null) {
      if (match[1] && match[1].length < 60) { // Safety length check
